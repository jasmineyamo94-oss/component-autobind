...
namespace JasmineYamo.ComponentAutoBind.Editor
{
    internal static class AutoBindInspectorDrawer
    {
        ...
        public static void DrawActionSection(
            Action onAutoBindAndValidate,
            Action onGenerateCode,
            Action onClearAll)
        {
            EditorGUILayout.BeginHorizontal();
            if (DrawTintButton("Auto Bind", GetPrimaryButtonColor(), GUILayout.Height(20f)))
            {
                onAutoBindAndValidate?.Invoke();
            }

            if (DrawTintButton("Generate Code", GetNeutralButtonColor(), GUILayout.Height(20f)))
            {
                onGenerateCode?.Invoke();
            }

            if (DrawTintButton("Clear", GetDangerButtonColor(), GUILayout.Height(20f), GUILayout.Width(55f)))
            {
                onClearAll?.Invoke();
            }
        ...
        public static void DrawSettingsSection(
            string namespaceValue,
            string classNameValue,
            string codePathValue)
        {
            DrawCompactInfoPair("NS", namespaceValue, "Class", classNameValue);
            DrawCompactInfoLine("Path", codePathValue);
        }

        public static void DrawKeyMapTip(
            AutoBindKeyMapSetting keyMapSetting,
            ref bool isExpanded)
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            isExpanded = EditorGUILayout.BeginFoldoutHeaderGroup(isExpanded, "Component Key Map");
            if (isExpanded)
            {
        ...
        public static void DrawBindComponentFoldout(
            SerializedProperty bindDatas,
            ref bool isExpanded,
            Action drawContent)
        {
            ...
            if (isExpanded)
            {
                if (bindCount == 0)
                {
                    EditorGUILayout.LabelField("No bindings yet.", EditorStyles.miniLabel);
                }
                else
                {
                    drawContent?.Invoke();
        ...
        public static int DrawBindDataList(SerializedProperty bindDatas)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("#", GetMiniHeaderStyle(), GUILayout.Width(18f));
            EditorGUILayout.LabelField("Binding Name", GetMiniHeaderStyle(), GUILayout.Width(BindNameWidth));
            EditorGUILayout.LabelField("Component", GetMiniHeaderStyle());
            EditorGUILayout.EndHorizontal();
            ...
            for (int i = 0; i < bindDatas.arraySize; i++)
            {
                ...
                bindComProperty.objectReferenceValue = EditorGUI.ObjectField(
                    objectRect,
        ...
        private static void DrawCompactInfoPair(
            string leftLabel,
            string leftValue,
            string rightLabel,
            string rightValue)
        {
            ...
            DrawCompactInfo(
                ...
                leftValue);
            DrawCompactInfo(
                new Rect(rect.x + halfWidth + 6f, rect.y, halfWidth, rect.height),
                rightLabel,
                rightValue);
        }
        ...
        private static void DrawCompactInfo(Rect rect, string label, string value)
        {
            ...
            EditorGUI.LabelField(
                valueRect,
                new GUIContent(value ?? string.Empty, value ?? string.Empty),
        ...
        private static void DrawKeyMapColumns(AutoBindKeyMapSetting keyMapSetting)
        {
            ...
            {
                EditorGUILayout.LabelField("No component mappings available.", EditorStyles.miniLabel);
                return;
            }
        ...
        private static bool DrawTintButton(string text, Color color, params GUILayoutOption[] options)
        {
            ...
            GUI.contentColor = Color.Lerp(oldContentColor, color, 0.8f);
        ...
        private static void DrawBindRowBackground(
            Rect rowRect,
            int index,
            bool hasMissingComponent,
            bool hasInvalidName)
        {
        ...
        private static string FormatKeyMapItem(KeyValuePair<string, string> item)
        {
            ...
            return
                $"<color=#{braceColor}>{{</color> <color=#{keyColor}>{item.Key}</color> "
                + $"<color=#{braceColor}>:</color> <color=#{valueColor}>{item.Value}</color> "
                + $"<color=#{braceColor}>}}</color>";
        }
        ...
        private static GUIStyle GetMiniHeaderStyle()
        {
            if (s_MiniHeaderStyle == null)
            {
                ...
                s_MiniHeaderStyle.normal.textColor = EditorGUIUtility.isProSkin
        ...
        private static GUIStyle GetTagStyle()
        {
            if (s_TagStyle == null)
            {
                ...
                s_TagStyle.normal.textColor = EditorGUIUtility.isProSkin
        ...
        private static GUIStyle GetDeleteButtonStyle()
        {
            if (s_DeleteButtonStyle == null)
            {
                ...
                s_DeleteButtonStyle.normal.textColor = EditorGUIUtility.isProSkin
                    ? new Color(0.88f, 0.74f, 0.74f)
                    : new Color(0.58f, 0.28f, 0.28f);
            }
